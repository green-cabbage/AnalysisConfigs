#!/depot/cms/users/yun79/conda_envs/hmm_pocket_coffea/bin/python3.10
# depot/users/yun79/conda_envs/hmm_pocket_coffea/bin/runner.py
print("""
...
def load_run_options(cfg):
    ...
    try:
        ...
        print(f"config.run_options: {config.run_options}")
    ...
    return config, config.run_options
...
if __name__ == '__main__':
    ...
    print(f"run_options['executor']: {run_options['executor']}")
    ...
    # DASK runners
        ...
        if "hmm" in run_options['executor']: # custom executor implementation
            log_folder = "dask_log"
            if "local" in run_options['executor']:
                print(f"local dask job submission with 32 workers")
                client = Client(
                    processes=True,
                    n_workers=16, # 32
            ...
            else: # slurm job submission on Dask Gateway
                print(f"hmm sl job submission to Dask Gateway")
                from dask_gateway import Gateway
                gateway = Gateway() # assume that the cluster is already made outside of this script
                cluster_info = gateway.list_clusters()[0]# get the first cluster by default. There only should be one anyways
                client = gateway.connect(cluster_info.name).get_client()
                
        else: #baseline pocket-coffea implementation
        ...
        performance_report_path = os.path.join(args.outputdir, log_folder)
        if not os.path.exists(performance_report_path):
            os.makedirs(performance_report_path)
       
        performance_report_path = performance_report_path + "/dask-report.html"
